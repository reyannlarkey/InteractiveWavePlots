...
fig.suptitle("Moving in Same Direction", color = 'white', fontsize = 24, fontweight = 10)
...
#-----------------Make initial waves---------------------
x0 = np.linspace(-6*np.pi,6*np.pi,1000)
time_array = np.arange(0, 100, 0.2)
...
def update_data(amp = 0.5,wavelen = 2, speed  = 1):
    ...
    for i, item in enumerate(time_array):
        ...
        c = amp * np.cos((2*np.pi/2)*(xoff*x0 - toff*item))
        a.append(s+c)
        si.append(s)
        co.append(c)
